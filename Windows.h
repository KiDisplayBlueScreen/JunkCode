...
BOOL CreateProcessSimple(LPTSTR CommandLine, BOOL InheritHandles, DWORD CreationFlags, LPSTARTUPINFO lpStartupInfo, LPPROCESS_INFORMATION lpProcessInformation)
{
	return CreateProcess(0, CommandLine, 0, 0, InheritHandles, CreationFlags, 0, 0, lpStartupInfo, lpProcessInformation);
}
HANDLE CreateThreadSimple(LPTHREAD_START_ROUTINE lpStartAddress, LPVOID lpParameter)
{
	return CreateThread(0, 0, lpStartAddress, lpParameter, 0, 0);
}
DWORD GetProcAddressByName(LPCWSTR lpModuleName, LPCSTR  lpProcName)
{
	LoadLibrary(lpModuleName);
	return (DWORD)GetProcAddress(GetModuleHandle(lpModuleName), lpProcName);
}